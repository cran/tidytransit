...
In order to work with time fields in the GTFS feed data, we first need to convert them to [`hms::hms()`](https://CRAN.R-project.org/package=hms). Times for the whole feed can be converted with `set_hms_times()`. See the `set_hms_times` reference for more detail on how it works. 
